...
namespace Lab2.AESLibrary
{
    public class AES
    {
        ...
        public void ECB(string filePath)
        {
            byte[] input = File.ReadAllBytes(filePath);

            byte[] encrypt = new byte[input.Length + 16];
            var watch = System.Diagnostics.Stopwatch.StartNew();
            Encrypt(input, ref encrypt);

        }

        public void CBC(string filePath)
        {
            byte[,] xor_vector = {
                { 0x54, 0x73, 0x20, 0x67},
                { 0x68, 0x20, 0x4b, 0x20},
            ...
            if (plaintext == null) //invalid input(s)
                throw new Exception();
            int extrabytes = plaintext.Length % BLOCK_SIZE;
            int pblock = plaintext.Length / BLOCK_SIZE + (extrabytes > 0 ? 1 : 0);
            ...
            { //Encrypt all possible blocks
                ...
                for (int i = 0; i < 4; ++i)
                {
                    for (int j = 0; j < 4; ++j)
                    {
                        text[i*4+j] = (byte)(plaintext[i * 4 + j] ^ xor_vector[j,i]);
                    }
                }
                encryptBlock(text, ref text);
                Array.Copy(text, 0, ciphertext, p, BLOCK_SIZE);
        ...
        private byte[,] ArrayToMatrix(byte[] x)
        {
        ...
        private byte[] MatrixToArray(byte[,] x)
        {
        ...
        public void CFB(string filePath)
        {
            byte[,] xor_vector = {
                { 0x54, 0x73, 0x20, 0x67},
                { 0x68, 0x20, 0x4b, 0x20},
            ...
            byte[] text = new byte[BLOCK_SIZE];
            int p;
            encryptBlock(MatrixToArray(xor_vector), ref text);
            xor_vector = ArrayToMatrix(text);
            for (int k = 0; k < pblock; ++k)
            { //Encrypt all possible blocks
                p = k * BLOCK_SIZE;
                for (int i = 0; i < 4; ++i)
        ...
        public void OFB(string filePath)
        {
            byte[,] xor_vector = {
                { 0x54, 0x73, 0x20, 0x67},
                { 0x68, 0x20, 0x4b, 0x20},
        ...
        public void CTR(string filePath)
        {
            byte[,] xor_vector = {
                { 0x54, 0x73, 0x20, 0x67},
                { 0x68, 0x20, 0x4b, 0x20},
            ...
            { //Encrypt all possible blocks
                ...
                encryptBlock(MatrixToArray(xor_vector), ref additional);

                xor_vector = ArrayToMatrix(additional);
